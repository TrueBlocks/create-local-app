...
**Trueblocks Dalle** is a Go package for generating, enhancing, and annotating creative prompts and images, powering the {{ORG_NAME}} Dalle application. It combines attribute-driven prompt generation, OpenAI integration, and image annotation in a modular, testable, and extensible design.
...
| `{{PROJECT_NAME}}.go` | DalleDress struct and prompt logic          |
...
```bash
git clone https://github.com/{{ORG_NAME}}/{{ORG_LOWER}}-dalle.git
cd {{ORG_LOWER}}-dalle
go mod tidy
...
```go
import "github.com/{{ORG_NAME}}/{{ORG_LOWER}}-dalle/v2/pkg/dalle"
